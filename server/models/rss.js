...
module.exports.init = async function init() {
  ...
  defer.setInterval(async function interval() {
...
module.exports.getItems = async function getItems(limit) {
  ...
  if (limit) {
    results = await r.table('rss').orderBy({ index: r.desc('timestamp') }).limit(limit);
  } else {
    results = await r.table('rss').orderBy({ index: r.desc('timestamp') });
  }
...
async function poll() {
  ...
  const all_feed_data = await Promise.all(rss_feed_promises);
  ...
  const feed_links = await _.map(formatted_all_feed_data, 'link');

  let old_feed_links = await r.table('rss').filter(
    (doc) => {
  ...
  const new_items = await _.filter(formatted_all_feed_data, (feed_item) => {
  ...
  let inserted_items = [];
  try {
    const results = await r.table('rss').insert(new_items, { returnChanges: true });
